...
import { GetDashBoardDTO, GetFinancialEvolutionDTO, IndexTransactionsDTO } from "../../dtos/transactions.dto";
import { Balance } from "../../entities/balance.entity";
import { Expense } from "../../entities/expense.entity";
import { Transaction, TrasactionType } from "../../entities/transactions.entity";
import { TransactionModel } from "../schemas/transactions.schemas";
...
export class TransactionsRepository {
    ...
    async create({ title, date, amount, type, category }: Transaction): Promise<Transaction> {
        const createdTransation = await this.model.create({ title, date, amount, type, category });

        return createdTransation.toObject<Transaction>();
    }
    ...
    async getBalance({ beginDate, endDate }: GetDashBoardDTO): Promise<Balance> {
        ...
        const [result] = await aggregate.project({
            _id: 0,
            income: {
                $cond: [
                    {
